...
import { IdAndMaybeVersion, IdAndVersion } from "../../../wikisim-core/src/data/id"
...
export function make_graph(data_map: DataComponentsMap, ids: KnowledgeGraphIds): Graph
{
    const { id_of_interest, id_of_concepts = id_of_interest, id_of_comparison } = ids

    const concept_graph = make_graph_inner(data_map, id_of_concepts)

    const graph_of_interest = (id_of_interest === id_of_concepts)
        ? concept_graph
        : make_graph_inner(data_map, id_of_interest)
    ...
    const comparison_graph = (id_of_comparison === id_of_concepts)
        ...
        : (id_of_comparison === id_of_interest)
            ? graph_of_interest
            : make_graph_inner(data_map, id_of_comparison)
...
function make_graph_inner(data_map: DataComponentsMap, top_level_id: number): Graph
{
